...
/** Dial codes starting at +1, then ascending by number. */
export const countryDialCodes: CountryDialCode[] = [
  ...
  { code: "TC", name: "Turks & Caicos", dial: "+1" },
...
export function getDialByCountryCode(code: string): string {
  return countryDialCodes.find((c) => c.code === code)?.dial ?? "+1";
}

export function formatInternationalPhone(countryCode: string, localNumber: string): string {
  const digits = localNumber.replace(/[^\d]/g, "");
  if (!digits) return "";
  const dial = getDialByCountryCode(countryCode);
  return `${dial} ${digits}`;
}
