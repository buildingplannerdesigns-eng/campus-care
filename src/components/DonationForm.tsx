...
export function DonationForm({ customCheckout = false }: { customCheckout?: boolean }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <button
        ...
        disabled={status === "submitting" || (customCheckout && !validAmount)}
        className={`${siteCtaClassName({ variant: "peach", className: "w-full px-6 py-4 sm:px-6 sm:py-4" })} disabled:cursor-not-allowed disabled:opacity-60`}
      >
        {status === "submitting"
          ? "Redirecting to Stripe…"
          : customCheckout
            ? `Donate ${validAmount ? `$${activeAmount}` : "Now"}${isMonthly ? " / month" : " today"}`
            : "Donate with Stripe"}
        <CtaArrow />
      </button>

      {customCheckout && honorGift ? (
        <p className="text-center text-xs text-parchment/50">
          Dedication details can be noted in the Stripe checkout message.
        </p>
      ) : null}

      {status === "error" && errorMessage ? (
        <p className="text-center text-sm font-medium text-[#b3421c]">{errorMessage}</p>
      ) : null}
